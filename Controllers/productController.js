...
const getCompleteProductData = async (productId) => {
  try {
    ...
    const getAttributesTypeQuery = "SELECT DISTINCT variationid, attributeprice FROM `attributes` WHERE productId = ?";
        ...
        if (error) {
          ...
          try {
            const variationsWithAttributeValues = await Promise.all(variations.map(async (variation) => {
              const getAttributeValuesQuery = "SELECT id,attributeValue, attributeType,attributeimg,attributestock FROM `attributes` WHERE productId = ? AND variationid = ?";
